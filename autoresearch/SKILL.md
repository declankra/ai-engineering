...
1. **README.md** in the benchmark directory — explains what this benchmark measures, the scoring methodology, and how to run it. Use the `BENCHMARK README TEMPLATE` in [TEMPLATES.md](TEMPLATES.md).

2. **program.md** — the program file built in Phase 4

2b. **projects/** directory — if the user has multiple datasets or segments, create a `projects/` directory with a subfolder per dataset. Each project folder should contain a README explaining review status and a gold label file. Use the `PROJECT README TEMPLATE` in [TEMPLATES.md](TEMPLATES.md).

3. **autoresearch/results.tsv** — empty ledger with headers. Use the `RESULTS.TSV TEMPLATE` in [TEMPLATES.md](TEMPLATES.md).
...
6. **autoresearch/README.md** — explains the autoresearch scaffold. Use the `AUTORESEARCH README TEMPLATE` in [TEMPLATES.md](TEMPLATES.md).
...
All file templates are in [TEMPLATES.md](TEMPLATES.md). Read that file when you reach Phase 4 or Phase 5 and need to create files. It contains templates for:

- `program.md` — the human-owned optimization spec with config block
- Benchmark README — scoring methodology and run commands
- Autoresearch README — scaffold documentation
- `results.tsv` — append-only experiment ledger with column definitions
- Project README — per-dataset review status and label schema
- Gold label structure — principles and JSON example for label files
